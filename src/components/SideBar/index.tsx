...
export const SideBar = (props: SideBarValues) => {
  const setImageClass = (i: number) => {
    if (i === props.activeSection) return "ActiveButton";
    else return "NonActiveButton";
  };
  return (
    <aside className="sideBarBody">
      <img
        className={setImageClass(0)}
        src={home}
        alt="Home"
        onClick={() => props.setActiveSection(0)}
      />
      <img
        className={setImageClass(1)}
        src={bell}
        alt="Notif"
        onClick={() => props.setActiveSection(1)}
      />
      <img
        className={setImageClass(2)}
        src={profile}
        alt="Profile"
        onClick={() => props.setActiveSection(2)}
      />
    </aside>
  );
};
